...
class EnvironmentOptions:
    ...
    def __repr__(self):
        ...
        """
        return (
            f"{self.__class__.__name__}(cluster_ip={self.cluster_ip},"
    ...
    def get_options(self):
        ...
        self.username = (
            args.username
            if args.username
            else input("Please enter your Prism Central username: ")
        )
...
class ApiClient:
    ...
    def __init__(self, cluster_ip, request, body, username, password, timeout=10):
    ...
    def send_request(self):
        ...
        try:
            ...
            print(
                "Connection timed out while connecting to "
            ...
            print(
                "An HTTP error occurred while connecting to "
                f"{self.cluster_ip}. Please check your connection, "
                "then try again."
            )
            sys.exit()
...
def generate_template(json_results):
    ...
    for json_result in json_results:
        ...
        for entity in json_result:
            if entity in supported_entities:
                ENTITY_TOTALS[f"{entity}"] = json_result[1]["metadata"]["total_matches"]
                print(
                    f"Count of entity type {entity}: "
        ...
        #########################
            ...
            for network_security_rule in json_result[1]["entities"]:
                ...
                HTML_ROWS["network_security_rule"] += (
                    f"<tr><td>{entity_name}" "</td></tr>"
                )
        #########
        # IMAGE #
        #########
            ...
            try:
                for image in json_result[1]["entities"]:
                    ...
                    HTML_ROWS["image"] += (
                        f"<tr><td>{entity_name}</td><td>" f"{image_type}" "</td></tr>"
                    )
            except KeyError:
                HTML_ROWS["vm"] += (
                    '<tr><td colspan="2">'
            ...
            try:
                for host in json_result[1]["entities"]:
                    if "name" in host["status"]:
                        host_serial = host["status"]["resources"]["serial_number"]
                        host_ip = host["status"]["resources"]["hypervisor"]["ip"]
                        cvm_ip = host["status"]["resources"]["controller_vm"]["ip"]
                        num_vms = host["status"]["resources"]["hypervisor"]["num_vms"]
                        HTML_ROWS["host"] += (
                            "<tr><td>"
                            f'{host["status"]["name"]}'
                            "</td><td>"
        ...
        ###########
            ...
            for cluster in json_result[1]["entities"]:
                try:
                    cluster_ip = (
                        (cluster["spec"]["resources"]["network"]["external_ip"])
                    ...
                    html_prefix = (
                        "AOS"
                        if (
                            cluster["status"]["resources"]["config"]["service_list"][0]
                            == "AOS"
                        )
                        else "Prism Central"
                    )

                    cluster_version = cluster["status"]["resources"]["config"]["build"][
                        "version"
                    ]
                    ...
                    is_ce = (
                        ...
                        if (
                            "-ce-"
                            in (
                                cluster["status"]["resources"]["config"]["build"][
                                    "full_version"
                                ]
                            )
                        )
            ...
            try:
                for blueprint in json_result[1]["entities"]:
                    ...
                    if not bool(blueprint["status"]["deleted"]):
                        ...
                        bp_project = (
                            (blueprint["metadata"]["project_reference"]["name"])
    ...
    template = source_html.safe_substitute(
        ...
        network_security_rule_total=str(ENTITY_TOTALS["network_security_rule"]),
